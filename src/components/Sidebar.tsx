...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  const { isTtsEnabled, toggleTts, t } = useTheme();
    ...
    >
      ...
      <div className="p-3">
        ...
        >
          ...
          {!isCollapsed && <span>{t('newChat') || 'New chat'}</span>}
        </button>
      </div>
      ...
      <div className="px-2 py-1 space-y-1">
        ...
        >
          ...
          {!isCollapsed && (
            <div className="flex items-center justify-between flex-1">
              <span>{t('textToSpeech') || 'Text-to-Speech'}</span>
              <span className={`text-[10px] font-extrabold uppercase px-1.5 py-0.5 rounded-full ${
                ...
                {isTtsEnabled ? (t('ttsOn') || 'ON') : (t('ttsOff') || 'OFF')}
              </span>
            </div>
          )}
        </button>
        ...
        >
          ...
          {!isCollapsed && <span>{t('navImages') || 'Images'}</span>}
        </button>
        ...
        >
          ...
          {!isCollapsed && <span>{t('navFeatures')}</span>}
        </button>
        ...
        >
          ...
          {!isCollapsed && <span>{t('navTips')}</span>}
        </button>
        ...
        >
          ...
          {!isCollapsed && <span>{t('symptomCheckBtn')}</span>}
        </button>
        ...
        >
          ...
          {!isCollapsed && <span>{t('navDiseases')}</span>}
        </button>
        ...
        >
          ...
          {!isCollapsed && <span>{t('navFAQ')}</span>}
        </button>
      </div>
      ...
      {!isCollapsed && (
        <div className="flex-1 overflow-y-auto px-3 py-3 space-y-1 border-t border-slate-200/60 dark:border-slate-800/60 mt-2">
          <div className="px-2 py-1 flex items-center justify-between text-[11px] font-extrabold uppercase text-slate-400 dark:text-slate-500 tracking-wider">
            ...
            <button onClick={onOpenHistory} className="hover:text-health-500 transition-colors">{t('viewAll') || 'View All'}</button>
          </div>
